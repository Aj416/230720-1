...
{
    public class SolveTicketCommandHandlerTest : BaseCommandHandlerTests<TicketCommandHandler>
    {
        public SolveTicketCommandHandlerTest()
        {
        ...
        [Fact]
        public async Task TicketStatusShouldChangeWhenTicketIsSolved()
        {
            // Arrange
            var id = Guid.NewGuid();
            var brandId = Guid.NewGuid();

            var cmd = new SolveTicketCommand(id);
            ...
            Assert.Equal(Domain.Enums.TicketStatusEnum.Solved, mockTicket.Status);
            Mocker.GetMock<IMediatorHandler>().Verify(x => x.RaiseEvent(It.IsAny<TicketSolvedEvent>()), Times.Once);

        }

		[Fact]
		public async Task TicketStatusShouldNotTransitionToSolvedStateWhenTagsAreEnabledAndNoneAreSet()
		{
			// Arrange
			var id = Guid.NewGuid();
			var brandId = Guid.NewGuid();
			...
			// Act
            await Assert.ThrowsAsync<DomainException>(async () => await SystemUnderTest.Handle(cmd, CancellationToken.None));
		}

        [Fact]
        public async Task TicketStatusShouldNotChangedWhenTicketIsSolvedButCurrentStatusNotAssigned()
        {
            // Arrange
            var id = Guid.NewGuid();
            var brandId = Guid.NewGuid();
