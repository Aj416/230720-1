...
using Tigerspike.Solv.Services.WebHook.Application;
using Tigerspike.Solv.Services.WebHook.Application.Consumers;
using Tigerspike.Solv.Services.WebHook.Configuration;
using Tigerspike.Solv.Services.WebHook.Extensions;
using Tigerspike.Solv.Services.WebHook.Infrastructure;
...
namespace Tigerspike.Solv.Services.WebHook
{
	public class Startup
	{
		...
		public virtual void ConfigureServices(IServiceCollection services)
		{
			...
			{
				configurator.AddConsumersFromNamespaceContaining<SendWebHookConsumer>();
		...
		public void Configure(IApplicationBuilder app, IWebHostEnvironment env)
		{
			...
			app.UseSwaggerDocs();
			app.UseServiceStack();
			app.ConfigureDynamoDb(createMissingTables: env.IsLocal() || env.IsDocker());
		...
		private List<BusEndpointConfiguration> GetEndpoints(IServiceProvider provider)
		{
			...
			return new List<BusEndpointConfiguration>
			{
				...
				{
					QueueName = options.Queues.WebHook,
					EndpointConfigurator = c =>
					{
						c.UseConcurrencyLimit(5);
						c.UseMessageRetry(r => r.Interval(5, TimeSpan.FromSeconds(5)));
						c.Consumer<StartWebHookTriggerConsumer>(provider);
						c.Consumer<SendWebHookConsumer>(provider);
						c.Consumer<CreateSubscriptionConsumer>(provider);
						c.Consumer<DeleteSubscriptionConsumer>(provider);
						c.Consumer<ChatMessageConsumer>(provider);

						EndpointConvention.Map<ISendWebHookCommand>(c.InputAddress);
					}
				}
			};
		}
