...
VALUES
	(@quizId, 'Final Evaluation Assessment', 'You have successfully viewed the ByteDance 101 guidelines.', 0, 'To Solv for ByteDance you''re going to need pass the assessment. Hit the button below to brush up on your brand knowledge or try the quiz again.', 'You''re now fully authorised with Dell, so go ahead, pick up your first ticket and start earning right now.');
...
VALUES
	...
	(UUID(), @questionId, 1, 'ByteDance', 2, 0);
...
INSERT INTO `brand` (
	...
	`ProbingFormId`)
VALUES (
	@brandId, 'ByteDance', NULL, NULL,
	'#78E6DC', -- Color
	'https://assets.solvnow.com/byte-dance/ByteDance.png', -- Thumbnail
	'https://assets.solvnow.com/byte-dance/ByteDance.png', -- Logo
	...
	'## CONGRATULATIONS! \n\n You have successfully viewed the ByteDance 101 guidelines. To represent ByteDance, you''ll now need to take a peer knowledge check.', -- InductionDoneMessage
	'ByteDance has put together this 101 guide to get you authorised to Solv for them.', -- InductionInstructions
	...
	'Chat with ByteDance', -- CreateTicketHeader
	'We are here to help you', -- CreateTicketSubheader
	'https://assets.solvnow.com/byte-dance/contract.txt', -- ContractUrl
	'https://assets.solvnow.com/byte-dance/contract.txt', -- ContractInternalUrl
...
VALUES
	(UUID(), 'Solv flow', 'https://assets.solvnow.com/byte-dance/solv-flow.html', 1, @inductionSectionId, 1);


-- api key
INSERT INTO `apikey` (`Id`, `BrandId`, `Key`, `UserId`, `CreatedDate`, `RevokedDate`, `ApplicationId`)
VALUES
	(UUID(), @brandId, 'byte-dance-1', NULL, UTC_TIMESTAMP(), NULL, 'byte-dance-1');
